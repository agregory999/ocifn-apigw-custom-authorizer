...
def handler(ctx, data: io.BytesIO = None):
    ...
    # Main routine
    logging.getLogger('assertion-generator').info("Enter JWT User Assertion Function")
    ...
    try:
        ...
        try:
            if cfg["DEBUG"] and cfg["DEBUG"].lower() == "true":
                ...
                logging.getLogger('assertion-generator').debug(f"DEBUG level set")
            else:
                logging.getLogger().setLevel(logging.INFO)
                logging.getLogger('assertion-generator').debug(f"INFO level set")
        except:
            logging.getLogger().setLevel(logging.INFO)
            logging.getLogger('assertion-generator').debug(f"INFO level set")
        ...
        try:
            ...
            logging.getLogger('assertion-generator').debug(f"IDCS Client ID: {idcs_appid}")
        except:
            raise
        ...
        try:
            ...
            logging.getLogger('assertion-generator').debug(f"Valid API Key: {validKey}")
        except:
            True
        ...
        if not validKey:
            ...
            logging.getLogger('assertion-generator').debug(f"No valid API Key - therefore not required as incoming")
        else:
            logging.getLogger('assertion-generator').debug(f"Valid API Key: {validKey}")
            if (validKey != apiKey):
        ...
        logging.getLogger('assertion-generator').debug('Current Time: ' + str(epoch_time) + ' : Expires: ' + str(exp_time))
        ...
        logging.getLogger('assertion-generator').debug(f"Assertion: {assertion}")
    ...
    # Generic Handler
        ...
        logging.getLogger('assertion-generator').error("Error: {0}".format(ex), exc_info=1)
        return response.Response(
            ctx, response_data=json.dumps(
            {"error": str(ex)}),
            headers={"Content-Type": "application/json"}
